...
| 27.4.2020 hotfix (estimated date) | New validation for disagreeToBePublished. The validtion allot th field to be set only F06 totalValue and finalTotalValue field and national contract notice estimatedValue fieids. Other use will result in validation errror in the ets api. The fix can be tested before production in staging environment | 
